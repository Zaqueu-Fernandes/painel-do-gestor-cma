import { TotaisRH } from '../services/supabase';
...
interface CardsRHProps {
  totais: TotaisRH;
}

const CardsRH = ({ totais }: CardsRHProps) => {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-6">
      <div className="bg-white p-5 rounded-xl shadow flex items-center gap-4">
        ...
        <div className="flex flex-col min-w-0">
          <span className="text-xs text-gray-500 font-medium uppercase">VALOR BRUTO</span>
          <span className="text-lg md:text-2xl font-bold text-gray-700 truncate">{formatarMoeda(totais.valorBruto)}</span>
        </div>
      </div>
      ...
      <div className="bg-white p-5 rounded-xl shadow flex items-center gap-4">
        ...
        <div className="flex flex-col min-w-0">
          <span className="text-xs text-gray-500 font-medium uppercase">VALOR LÍQUIDO</span>
          <span className="text-lg md:text-2xl font-bold text-gray-700 truncate">{formatarMoeda(totais.valorLiquido)}</span>
        </div>
      </div>
    </div>
  );
