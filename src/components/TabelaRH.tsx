...
interface TabelaRHProps {
  dados: any[];
}

const TabelaRH = ({ dados }: TabelaRHProps) => {
  ...
  const ordenarDados = (dados: any[], coluna: string, ordem: string) => {
    ...
    dadosClone.sort((a, b) => {
      let valorA = a[coluna];
      let valorB = b[coluna];
      if (coluna === 'data') { valorA = new Date(valorA); valorB = new Date(valorB); }
    ...
    return (
      <div className="flex justify-center items-center gap-2 mt-5">
        ...
        {Array.from({ length: fimBtn - inicioBtn + 1 }, (_, i) => inicioBtn + i).map((num) => (
          <button key={num} onClick={() => mudarPagina(num)} className={`px-3 py-2 border rounded-md cursor-pointer transition-all min-w-[40px] ${paginaAtual === num ? 'bg-green-800 text-white border-green-800' : 'border-gray-200 bg-white hover:bg-green-800 hover:text-white hover:border-green-800'}`}>
            {num}
          </button>
        ))}
        {fimBtn < totalPaginas && (
        ...
        <button onClick={() => mudarPagina(paginaAtual + 1)} disabled={paginaAtual === totalPaginas} className="px-3 py-2 border border-gray-200 bg-white rounded-md cursor-pointer transition-all min-w-[40px] hover:bg-green-800 hover:text-white hover:border-green-800 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-white disabled:hover:text-gray-700 disabled:hover:border-gray-200">
          ...
          <thead className="bg-green-800 text-white sticky top-0 z-10">
            <tr>
              {[
                ...
                { key: 'mes_competencia', label: 'Competência' },
                { key: 'categoria', label: 'Categoria' },
                { key: 'vinculo', label: 'Vínculo' },
                { key: 'servidor', label: 'Servidor' },
                { key: 'descricao', label: 'Descrição' },
              ].map(col => (
                <th key={col.key} onClick={() => handleOrdenar(col.key)} className="p-3.5 text-left font-semibold cursor-pointer whitespace-nowrap hover:bg-green-700">
                  {col.label} <i className="fas fa-sort ml-1 text-xs"></i>
                </th>
              ))}
              <th className="p-3.5 text-left font-semibold">Valor Bruto</th>
              <th className="p-3.5 text-left font-semibold">Deduções</th>
              <th className="p-3.5 text-left font-semibold">Valor Líquido</th>
              <th className="p-3.5 text-left font-semibold">Anexo</th>
            </tr>
          </thead>
          <tbody>
            {dadosPagina.map((item, index) => (
              <tr key={index} className="border-b border-gray-200 transition-colors hover:bg-gray-50">
                ...
                <td className="p-3">{formatarMoeda(item.valor_bruto)}</td>
                <td className="p-3">{formatarMoeda(item.deducoes)}</td>
                <td className="p-3">{formatarMoeda(item.valor_liquido)}</td>
                <td className="p-3">
                  {item.anexo ? (
                    <a href={item.anexo} target="_blank" rel="noopener noreferrer" className="text-blue-500 cursor-pointer transition-colors hover:text-blue-700">
